...
test.describe('Template Integration', () => {
  ...
  test('should browse and preview available templates', async ({ page }) => {
    ...
    const categories = await page.locator('[data-testid="category-option"]').all();
    ...
    await expect(previewModal.locator('[data-testid="template-name"]')).toBeVisible();
    await expect(previewModal.locator('[data-testid="template-description"]')).toBeVisible();
    await expect(previewModal.locator('[data-testid="template-workflow"]')).toBeVisible();
  });
  ...
  test('should instantiate template with parameters', async ({ page }) => {
    ...
    const parameterForm = page.locator('[data-testid="template-parameters-form"]');
    await expect(parameterForm).toBeVisible();

    // Fill required parameters
    await page.locator('[data-testid="param-company-name"]').fill('Test Company');
    await page.locator('[data-testid="param-support-email"]').fill('support@testcompany.com');
  ...
  test('should validate template parameters', async ({ page }) => {
    ...
    const createButton = page.locator('[data-testid="create-from-template-button"]');
    ...
    await page.locator('[data-testid="param-support-email"]').fill('invalid-email');
    await createButton.click();
  ...
  test('should search and filter templates', async ({ page }) => {
    ...
    const searchResults = await page.locator('[data-testid="template-card"]').all();
    ...
    const filteredResults = await page.locator('[data-testid="template-card"]').all();
    
    // Verify filtered results are from support category
    for (const result of filteredResults) {
      const category = await result.locator('[data-testid="template-category"]').textContent();
  ...
  test('should handle template creation workflow', async ({ page }) => {
    ...
    await builder.goto();
    await builder.createNewWorkflow('Custom Template', 'A custom workflow to save as template');
    ...
    // Save as template
    const saveTemplateButton = page.locator('[data-testid="save-as-template-button"]');
    ...
    await page.locator('[data-testid="template-name-input"]').fill('My Custom Template');
    await page.locator('[data-testid="template-description-input"]').fill('A custom template for testing');
    await page.locator('[data-testid="template-category-select"]').selectOption('custom');
    ...
    await expect(page.locator('[data-testid="template-success-message"]')).toBeVisible();

    // Navigate to templates and verify it's there
    await page.goto('/templates');
    const customTemplate = page.locator('[data-testid="template-card"]')
