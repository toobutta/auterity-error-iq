...
test.describe('Cross-System Integration', () => {
  ...
  test('should integrate AutoMatrix with RelayCore for AI routing', async ({ page }) => {
    ...
    await page.locator('[data-testid="cost-optimization"]').selectOption('balanced');
    await page.locator('[data-testid="performance-priority"]').selectOption('medium');
    ...
    await toolbox.locator('[data-testid="node-ai_process"]').dragTo(canvas, { targetPosition: { x: 200, y: 200 } });
    ...
    await expect(page.locator('[data-testid="routing-active-indicator"]')).toBeVisible();
  ...
  test('should integrate with NeuroWeaver for model specialization', async ({ page }) => {
    // Navigate to NeuroWeaver interface
    await page.goto('/models');
    await expect(page).toHaveURL(/.*models/);

    // Check NeuroWeaver dashboard loads
    await expect(page.locator('[data-testid="models-dashboard"]')).toBeVisible();
    ...
    const modelCards = await modelsGrid.locator('[data-testid="model-card"]').all();
    ...
    await expect(modelDetails.locator('[data-testid="model-name"]')).toBeVisible();
    await expect(modelDetails.locator('[data-testid="model-performance"]')).toBeVisible();
  ...
  test('should display unified monitoring across all systems', async ({ page }) => {
    ...
    const healthIndicators = await page.locator('[data-testid="health-indicator"]').all();
    ...
    const metricsValue = await page.locator('[data-testid="current-requests"]').textContent();
    
    // Wait for potential update (metrics should update)
    await page.waitForTimeout(5000);
    
    // Check if value updated (in a real scenario with active traffic)
    const updatedValue = await page.locator('[data-testid="current-requests"]').textContent();
  ...
  test('should handle cross-system error propagation', async ({ page }) => {
    ...
    await toolbox.locator('[data-testid="node-data_input"]').dragTo(canvas, { targetPosition: { x: 100, y: 200 } });
    await toolbox.locator('[data-testid="node-ai_process"]').dragTo(canvas, { targetPosition: { x: 300, y: 200 } });
    await toolbox.locator('[data-testid="node-model_inference"]').dragTo(canvas, { targetPosition: { x: 500, y: 200 } });

    // Connect nodes
    await page.locator('[data-testid="node-data_input-1"] [data-testid="output-handle"]')
      .dragTo(page.locator('[data-testid="node-ai_process-1"] [data-testid="input-handle"]'));
    
    await page.locator('[data-testid="node-ai_process-1"] [data-testid="output-handle"]')
      .dragTo(page.locator('[data-testid="node-model_inference-1"] [data-testid="input-handle"]'));
    ...
    await expect(page.locator('[data-testid="execution-error"]')).toBeVisible({ timeout: 10000 });
  ...
  test('should synchronize user authentication across systems', async ({ page }) => {
    ...
    const permissions = await page.locator('[data-testid="user-permission"]').all();
