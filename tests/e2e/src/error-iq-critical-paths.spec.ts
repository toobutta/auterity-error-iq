...
test.describe('Error Monitoring Dashboard - Critical Paths', () => {
  ...
  test.describe('KPI Header & Real-time Updates', () => {
    test('should display key metrics with proper formatting', async ({ page }) => {
      // Check that all KPI cards are present
      const kpiCards = page.locator('[data-testid="kpi-card"]');
      await expect(kpiCards).toHaveCount(4);

      // Verify metric formatting (numbers with K/M suffixes)
      const errorCountCard = page.locator('[data-testid="kpi-card-error-count"]');
      const errorCountValue = errorCountCard.locator('[data-testid="metric-value"]');
      await expect(errorCountValue).toContainText(/^\d+(\.\d+)?[KM]?$/);

      // Check trend indicators are present
      const trendIndicator = errorCountCard.locator('[data-testid="trend-indicator"]');
    ...
    test('should show severity-appropriate colors for critical issues', async ({ page }) => {
      const criticalCard = page.locator('[data-testid="kpi-card-critical-errors"]');
      await expect(criticalCard).toHaveClass(/border-l-red-500/);
      
      const severityBadge = criticalCard.locator('[data-testid="severity-badge"]');
  ...
  test.describe('Issue List & Filtering', () => {
    test('should display paginated list of issues', async ({ page }) => {
      ...
      await expect(firstIssue.locator('[data-testid="severity-badge"]')).toBeVisible();
      await expect(firstIssue.locator('[data-testid="status-badge"]')).toBeVisible();
    });
    ...
    test('should filter issues by severity', async ({ page }) => {
      ...
      const initialCount = await page.locator('[data-testid="issue-count"]').textContent();
      ...
      await page.waitForFunction(() => {
        const countElement = document.querySelector('[data-testid="issue-count"]');
        return countElement && countElement.textContent !== initialCount;
      });
      
      // Verify only critical issues are shown
      const visibleSeverityBadges = page.locator('[data-testid="severity-badge"]:visible');
      const severityTexts = await visibleSeverityBadges.allTextContents();
      expect(severityTexts.every(text => text.toLowerCase().includes('critical'))).toBe(true);
    });
    ...
    test('should search issues by title/description', async ({ page }) => {
      ...
      const issueTitles = await issueRows.locator('[data-testid="issue-title"]').allTextContents();
      expect(issueTitles.some(title => 
        title.toLowerCase().includes('database') || title.toLowerCase().includes('timeout')
      )).toBe(true);
    });
    ...
    test('should handle empty search results gracefully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="empty-state-message"]')).toContainText('No issues found');
    });

    test('should support bulk actions for issue management', async ({ page }) => {
      await page.goto('/issues');
      await page.waitForSelector('[data-testid="issue-list"]');
      ...
      await page.selectOption('[data-testid="bulk-status-select"]', 'investigating');
  ...
  test.describe('Issue Detail View', () => {
    test('should navigate to issue detail from list', async ({ page }) => {
      ...
      const issueTitle = await firstIssue.locator('[data-testid="issue-title"]').textContent();
      
      await firstIssue.click();
      
      // Verify navigation to detail page
      await expect(page).toHaveURL(/\/issues\/[^\/]+$/);
      await expect(page.locator('[data-testid="issue-detail-title"]')).toContainText(issueTitle);
    });
    ...
    test('should display comprehensive issue information', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="issue-overview"]')).toBeVisible();
      await expect(page.locator('[data-testid="impact-metrics"]')).toBeVisible();
      await expect(page.locator('[data-testid="issue-timeline"]')).toBeVisible();
      await expect(page.locator('[data-testid="stack-trace"]')).toBeVisible();
      
      // Check impact metrics are properly formatted
      const affectedUsers = page.locator('[data-testid="affected-users-count"]');
      await expect(affectedUsers).toContainText(/^\d+(\.\d+)?[KM]?\s+users?$/);
    });

    test('should allow status changes with proper workflow', async ({ page }) => {
      await page.goto('/issues/test-issue-123');
      await page.waitForSelector('[data-testid="issue-detail"]');
      ...
      await expect(page.locator('[data-testid="timeline-event"]').first()).toContainText('Status changed to investigating');
    });

    test('should expand stack trace frames with code context', async ({ page }) => {
      await page.goto('/issues/test-issue-123');
      await page.waitForSelector('[data-testid="issue-detail"]');
      ...
      await expect(page.locator('[data-testid="error-line"]')).toHaveClass(/bg-red-100/);
    });
    ...
    test('should show related issues and suggestions', async ({ page }) => {
      ...
      if (await relatedIssues.isVisible()) {
        const relatedLinks = relatedIssues.locator('[data-testid="related-issue-link"]');
  ...
  test.describe('Performance & Responsiveness', () => {
    ...
    test('should handle large issue lists without performance degradation', async ({ page }) => {
  ...
  test.describe('Accessibility', () => {
    ...
    test('should have proper ARIA labels and roles', async ({ page }) => {
      ...
      await expect(kpiSection).toHaveAttribute('aria-label', /metrics|dashboard/i);
    ...
    test('should meet color contrast requirements', async ({ page }) => {
      ...
      if (await criticalBadge.isVisible()) {
        ...
        expect(styles.backgroundColor).toMatch(/rgb\(254, 242, 242\)|rgb\(239, 68, 68\)/); // Light red bg
      }
    });
  });
  ...
  test.describe('Error States & Edge Cases', () => {
    test('should handle API errors gracefully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="error-message"]')).toContainText(/failed to load|error/i);
    ...
    test('should handle very long issue titles gracefully', async ({ page }) => {
