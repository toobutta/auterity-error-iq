...
test.describe('Workflow Lifecycle', () => {
  ...
  test('should create, execute, and monitor a simple workflow', async ({ page }) => {
    ...
    // Configure each node
    await builder.configureNode('data_input-1', testWorkflows.simple.steps[0].config);
    await builder.configureNode('ai_process-1', testWorkflows.simple.steps[1].config);
    await builder.configureNode('output-1', testWorkflows.simple.steps[2].config);
    ...
    const createdWorkflow = recentWorkflows.find(wf => wf.name === testWorkflows.simple.name);
    expect(createdWorkflow).toBeTruthy();
  });

  test('should handle complex workflow with branching logic', async ({ page }) => {
    const builder = new WorkflowBuilderPage(page);
  ...
  test('should prevent execution of invalid workflows', async ({ page }) => {
    ...
    await builder.goto();
    await builder.createNewWorkflow('Invalid Workflow', 'A workflow with missing connections');
  ...
  test('should display workflow execution progress', async ({ page }) => {
    ...
    await builder.goto();
    await builder.createNewWorkflow('Progress Test', 'Test workflow execution progress');
    ...
    await builder.configureNode('data_input-1', { input_type: 'text', value: 'Test data' });
    await builder.configureNode('ai_process-1', { prompt: 'Process this', model: 'gpt-3.5-turbo' });
