...
export class DashboardPage {
  ...
  constructor(page: Page) {
    ...
    this.recentWorkflowsList = page.locator(
      '[data-testid="recent-workflows-list"]',
    );
    this.createWorkflowButton = page.locator(
      '[data-testid="create-workflow-btn"]',
    );
    this.browseTemplatesButton = page.locator(
  ...
  async getWorkflowCount() {
    const countElement = this.workflowsCard.locator(
  ...
  async getTemplateCount() {
    const countElement = this.templatesCard.locator(
  ...
  async getRecentWorkflows() {
    const workflowItems = await this.recentWorkflowsList
    ...
    for (const item of workflowItems) {
      ...
      const status = await item
        ...
        .textContent();
      const lastRun = await item
        .locator('[data-testid="workflow-last-run"]')
        .textContent();
