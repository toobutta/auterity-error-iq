...
export class WorkflowBuilderPage {
  ...
  constructor(page: Page) {
    ...
    this.workflowNameInput = page.locator(
      '[data-testid="workflow-name-input"]',
    );
    this.workflowDescriptionInput = page.locator(
  ...
  async connectNodes(sourceNodeId: string, targetNodeId: string) {
    const sourceNode = this.canvas.locator(
      `[data-testid="node-${sourceNodeId}"]`,
    );
    const targetNode = this.canvas.locator(
      `[data-testid="node-${targetNodeId}"]`,
    );
  ...
  async configureNode(nodeId: string, config: Record<string, any>) {
    ...
    for (const [key, value] of Object.entries(config)) {
      const input = this.propertiesPanel.locator(
  ...
  async validateWorkflow() {
    const validateButton = this.page.locator(
  ...
  async getWorkflowConnections() {
    const connections = await this.canvas
