...
export const ErrorToast: React.FC<ErrorToastProps> = ({ 
  ...
  return (
    <div className={`fixed ${positionClass} max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden z-50 ${styles.border}`}>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {error.retryable && onRetry && (
                  <button
                    onClick={handleRetry}
                    disabled={isRetrying}
                    className="text-xs bg-white bg-opacity-80 hover:bg-opacity-100 px-2 py-1 rounded border border-current disabled:opacity-50"
                  >
                ...
                {recoveryActions.map((action, index) => (
                  <button
                    ...
                    className={`text-xs px-2 py-1 rounded border border-current ${
                      action.primary 
                ...
                {onReport && (
                  <button
                    onClick={onReport}
                    className="text-xs bg-transparent hover:bg-white hover:bg-opacity-20 px-2 py-1 rounded border border-current"
                  >
                    Report
                  </button>
                )}
