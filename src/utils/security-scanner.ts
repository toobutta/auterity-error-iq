/**
 * Security Scanner for Chrome DevTools Integration
...
class SecurityScanner {
  ...
  private async scanForVulnerabilities(): Promise<SecurityIssue[]> {
    ...
    scripts.forEach((script, index) => {
      ...
      const integrity = (script as HTMLScriptElement).integrity;

      if (src && !src.startsWith(window.location.origin) && !integrity) {
        issues.push({
          id: `external-script-no-integrity-${index}`,
          severity: 'medium',
          title: 'External Script Without Integrity Check',
