...
export class EnterpriseAPIGateway {
  ...
  private async validateAPIKey(apiKey: string): Promise<boolean> {
    try {
      ...
      await this.redis.setex(`api_key:${apiKey}`, 300, isValid ? 'valid' : 'invalid');
  ...
  private setupManagementRoutes(): void {
    ...
    this.app.get('/api/gateway/services', async (req, res) => {
      ...
      for (const [name, config] of Object.entries(this.config.upstream_services)) {
        try {
  ...
  private setupProxyRoutes(): void {
    ...
    this.app.use('/api/v1/workflows', createProxyMiddleware({
      target: this.config.upstream_services.autmatrix?.url || 'http://localhost:8000',
      changeOrigin: true,
    ...
    // RelayCore (AI Router)
    this.app.use('/api/v1/ai', createProxyMiddleware({
      target: this.config.upstream_services.relaycore?.url || 'http://localhost:3001',
      changeOrigin: true,
    ...
    this.app.use('/api/v1/models', createProxyMiddleware({
      target: this.config.upstream_services.neuroweaver?.url || 'http://localhost:8080',
      changeOrigin: true,
      pathRewrite: { '^/api/v1/models': '/api/v1/models' },
      onError: (err, req, res) => {
        logger.error('NeuroWeaver proxy error', { error: err.message });
        res.status(502).json({ error: 'Model service temporarily unavailable' });
      },
    }));
  }
