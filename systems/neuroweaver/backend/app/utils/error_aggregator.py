...
class NeuroWeaverErrorAggregator:
            ...
            async with self.get_session() as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        self.logger.debug(f"Successfully aggregated error: {error_data.get('code')}")
                        return True
                    else:
                        error_text = await response.text()
                        self.logger.error(f"Failed to aggregate error: {response.status} - {error_text}")
                        return False
    ...
    async def aggregate_inference_error(
        ...
        try:
            ...
            sanitized_input = self._sanitize_input_data(input_data) if input_data else None
        ...
        try:
            ...
            async with self.get_session() as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        self.logger.info(f"Successfully aggregated batch of {len(errors)} errors")
                        return True
                    else:
                        error_text = await response.text()
                        self.logger.error(f"Failed to aggregate batch errors: {response.status} - {error_text}")
                        return False
    ...
    def _get_error_severity(self, error: Exception) -> str:
        ...
        if ('memory' in error_message or 'oom' in error_message or
            'cuda' in error_message or 'gpu' in error_message or
            'timeout' in error_message or 'connection' in error_message):
            return 'critical'
        
        # High severity errors
        if ('training' in error_type or 'deployment' in error_type or
            'model' in error_message or 'inference' in error_message):
            return 'high'
