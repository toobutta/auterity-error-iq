...
class RelayCoreConnector:
    ...
    async def register_service(self) -> bool:
        ...
        try:
            ...
            success = await self._make_relaycore_request(
                "POST", "/api/v1/providers/register", service_info
            )
    ...
    async def register_model(self, model: ModelInfo) -> bool:
        ...
        try:
            if not self.connection_healthy:
                logger.warning(
                    "RelayCore connection not healthy, skipping model registration"
                )
                return False
            ...
            success = await self._make_relaycore_request(
                "POST", "/api/v1/models/register", asdict(registration_data)
            )
    ...
    async def unregister_model(self, model_id: str) -> bool:
        ...
        try:
            if not self.connection_healthy:
                logger.warning(
                    "RelayCore connection not healthy, skipping model unregistration"
                )
                return False
            ...
            if success:
                ...
                logger.info(
                    f"Successfully unregistered model {model_id} from RelayCore"
                )
                return True
            else:
    ...
    async def update_model_performance(self, model_id: str, metrics: Dict) -> bool:
        ...
        try:
            ...
            success = await self._make_relaycore_request(
                "PUT", f"/api/v1/models/{model_id}/performance", update_data
            )

            if success:
                logger.info(
                    f"Updated performance metrics for model {model_id} in RelayCore"
                )
                return True
            else:
                logger.error(
                    f"Failed to update performance metrics for model {model_id}"
                )
                return False
    ...
    async def notify_model_status_change(
        self, model_id: str, status: str, reason: str = ""
    ) -> bool:
        ...
        try:
            ...
            success = await self._make_relaycore_request(
                "POST", f"/api/v1/models/{model_id}/status", notification_data
            )

            if success:
                logger.info(
                    f"Notified RelayCore of status change for model {model_id}: {status}"
                )
                return True
            else:
                logger.error(
                    f"Failed to notify RelayCore of status change for model {model_id}"
                )
                return False
    ...
    async def get_routing_analytics(self, model_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            response = await self._make_relaycore_request(
                "GET", f"/api/v1/models/{model_id}/analytics"
            )
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.request(
                    ...
                    if response.status < 400:
                        ...
                        logger.error(
                            f"RelayCore request failed: {response.status} - {error_text}"
                        )
                        return None
