...
interface RuleCondition {
  field: string;
  operator: 'equals' | 'exists' | 'length_less_than' | 'length_greater_than' | 'contains';
  value?: any;
}
...
export class SteeringRulesEngine {
  ...
  async loadSteeringRules(): Promise<void> {
    try {
      ...
      logger.info('Steering rules loaded successfully', { rulesCount: this.config.routing_rules.length });
    } catch (error) {
  ...
  async validateRules(): Promise<boolean> {
    ...
    try {
      ...
      if (!this.config.routing_rules || !Array.isArray(this.config.routing_rules)) {
  ...
  async determineRouting(request: AIRequest, profileId: string): Promise<RoutingDecision> {
    ...
    try {
      ...
      for (const rule of this.config!.routing_rules) {
        if (this.evaluateProfileConditions(rule.conditions, request, profileId)) {
          ...
          if (decision.estimated_cost > this.config!.cost_constraints.per_request_max) {
            logger.warn(`Request cost ${sanitizeForLog(decision.estimated_cost)} exceeds max ${sanitizeForLog(this.config!.cost_constraints.per_request_max)}`);
      ...
      return this.createFallbackDecision('Steering engine error', ['error_fallback']);
    }
  }
    ...
    return conditions.every(condition => {
      ...
      switch (condition.operator) {
        ...
        case 'length_less_than':
          return typeof fieldValue === 'string' && fieldValue.length < condition.value;
        case 'length_greater_than':
          return typeof fieldValue === 'string' && fieldValue.length > condition.value;
        case 'contains':
          return typeof fieldValue === 'string' && fieldValue.includes(condition.value);
        default:
          logger.warn(`Unknown operator: ${sanitizeForLog(condition.operator)}`);
          return false;
      }
    });
  }
  ...
  private createFallbackDecision(reason: string, rulesApplied: string[]): RoutingDecision {
    return {
      provider: 'openai',
      model: 'gpt-3.5-turbo',
