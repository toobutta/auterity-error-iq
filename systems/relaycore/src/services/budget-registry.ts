...
export class BudgetRegistry {
   ...
   */
  async createBudget(request: CreateBudgetRequest, createdBy: string): Promise<BudgetDefinition> {
    ...
    try {
      ...
      if (!endDate && request.period !== 'custom') {
        endDate = this.calculatePeriodEndDate(request.startDate, request.period);
      }
      ...
      logger.info(`Budget created: ${budgetId} for ${request.scopeType}:${request.scopeId}`);
  ...
  async getBudget(budgetId: string): Promise<BudgetDefinition | null> {
    try {
      const query = 'SELECT * FROM budget_definitions WHERE id = $1 AND active = true';
   ...
   */
  async updateBudget(budgetId: string, updates: UpdateBudgetRequest): Promise<BudgetDefinition | null> {
    const client = await DatabaseConnection.getClient();
    ...
    try {
      ...
      if (updates.amount !== undefined) {
        await client.query('SELECT refresh_budget_status_cache($1)', [budgetId]);
      }
  ...
  async deleteBudget(budgetId: string): Promise<boolean> {
    ...
    try {
      ...
      await client.query('DELETE FROM budget_status_cache WHERE budget_id = $1', [budgetId]);
   ...
   */
  async getBudgetHierarchy(scopeType: ScopeType, scopeId: string): Promise<BudgetDefinition[]> {
    try {
      ...
      const result = await DatabaseConnection.query(query, [scopeType, scopeId]);
