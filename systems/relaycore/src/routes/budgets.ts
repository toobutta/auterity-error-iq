...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!createRequest.name || !createRequest.scopeType || !createRequest.scopeId || 
        !createRequest.amount || !createRequest.currency || !createRequest.period || 
        !createRequest.startDate) {
      return res.status(400).json({
        error: 'Missing required fields',
        required: ['name', 'scopeType', 'scopeId', 'amount', 'currency', 'period', 'startDate']
      });
    }
...
router.get('/hierarchy/:type/:id', async (req: Request, res: Response) => {
  try {
    ...
    const hierarchy = await budgetRegistry.getBudgetHierarchy(scopeType as any, scopeId);
...
router.post('/:id/usage', async (req: Request, res: Response) => {
  try {
    ...
    // Validate required fields
    if (!usageRequest.amount || !usageRequest.currency || !usageRequest.source) {
      return res.status(400).json({
        error: 'Missing required fields',
...
router.post('/:id/check-constraints', async (req: Request, res: Response) => {
  try {
    ...
    const constraintCheck = await budgetTracker.checkBudgetConstraints(budgetId, estimatedCost);
...
router.get('/:id/usage', async (req: Request, res: Response) => {
  try {
    ...
    const usageHistory = await budgetTracker.getUsageHistory(budgetId, limit, offset);
...
router.get('/:id/usage/summary', async (req: Request, res: Response) => {
  try {
    ...
    const summary = await budgetTracker.getUsageSummary(budgetId, startDate, endDate);
