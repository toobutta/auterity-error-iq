 ...
 */
router.post('/feedback', csrfProtection, async (req: Request, res: Response): Promise<void> => {
  try {
    const { modelId, requestId, accuracy, latency, userSatisfaction, contextMatch } = req.body;

    if (!modelId || !requestId || accuracy === undefined || !latency) {
      res.status(400).json({
        success: false,
        error: { message: 'Missing required fields: modelId, requestId, accuracy, latency' }
      });
      return;
    }
 ...
 */
router.post('/switch', csrfProtection, async (req: Request, res: Response): Promise<void> => {
  try {
    const { currentModel, targetModel, reason, switchType = 'immediate' } = req.body;
    ...
    res.json({
      success,
      message: success ? 'Model switch requested' : 'Model switch request failed'
    });
  } catch (error) {
 ...
 */
router.get('/models/:modelId/health', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.error(`Failed to get model health for ${req.params.modelId}:`, error);
    res.status(500).json({
      success: false,
 ...
 */
router.put('/models/:modelId/thresholds', csrfProtection, async (req: Request, res: Response): Promise<void> => {
  try {
    const { modelId } = req.params;
    const { min_accuracy, max_latency_ms, min_throughput_rps, max_cost_per_request } = req.body;
    ...
    logger.error(`Failed to update thresholds for ${req.params.modelId}:`, error);
    res.status(500).json({
      success: false,
