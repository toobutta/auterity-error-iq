...
export const SystemStatus: React.FC<SystemStatusProps> = ({ status, connected, onAdminCommand }) => {
      ...
      {status ? (
        <div className="space-y-3">
          <div className="flex justify-between">
            ...
            <span className={`font-semibold ${status.system_health === 'healthy' ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="flex justify-between">
            ...
            <span className="text-white">{formatMemory(status.memory_usage?.heapUsed || 0)}</span>
          </div>
          
          <div className="flex justify-between">
            <span className="text-gray-400">Last Updated:</span>
            <span className="text-white">{new Date(status.last_updated).toLocaleTimeString()}</span>
          </div>
        </div>
      ) : (
