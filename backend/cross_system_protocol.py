...
class MessageBroker:
    ...
    async def _deliver_at_most_once(self, message: Message, handlers: List[MessageHandler]) -> bool:
    ...
    async def _deliver_at_least_once(self, message: Message, handlers: List[MessageHandler]) -> bool:
    ...
    async def _deliver_exactly_once(self, message: Message, handlers: List[MessageHandler]) -> bool:
...
class SystemConnector:
    ...
    async def send_command(self, target_system: str, command: str, payload: Dict[str, Any]) -> Optional[Message]:
    ...
    async def query(self, target_system: str, query: str, parameters: Dict[str, Any]) -> Optional[Message]:
        ...
        class ResponseHandler:
            async def handle(self, response_msg: Message) -> Optional[Message]:
                if (response_msg.correlation_id == correlation_id and 
...
async def demonstrate_cross_system_communication():
    ...
    # Send command
    start_time = time.time()
    command_msg = await api_gateway.send_command("order_service", "create_order", {
    ...
    # Query order status
    start_time = time.time()
    response = await api_gateway.query("order_service", "get_order_status", {
        "order_id": "order-123"
    })
    ...
    # Publish event
    await order_service.publish_event("order_status_changed", {
        "order_id": "order-123",
        "new_status": "shipped"
    })
