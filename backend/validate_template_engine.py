...
class MockTemplate:
    ...
    def __init__(
        ...
        parameters: List[MockTemplateParameter] = None,
    ):
        self.id = uuid.uuid4()
        self.name = name
...
def test_template_functionality():
    ...
    template_definition = {
        "nodes": [
            {"id": "start", "type": "start", "data": {"label": "Start: {{greeting}}"}},
            {
                "id": "process",
                "type": "ai_process",
                "data": {
    ...
    complex_definition = {
        ...
        "steps": [{"type": "condition", "condition": "{{enable_feature}}"}],
    }
    ...
    dealership_result = substitute_parameters(
        customer_inquiry_template, dealership_params
    )
