...
async def test_error_analytics():
    ...
    try:
        ...
        test_errors = [
            ...
            WorkflowError(
                "Step execution failed", workflow_id="test-123", step_name="ai_process"
            ),
        ...
        logger.info(
            f"Error metrics: {metrics.total_errors} errors, rate: {metrics.error_rate:.2f}/hour"
        )
...
async def test_enhanced_recovery():
    ...
    try:
        ...
        plan = await recovery_service.create_recovery_plan(
            test_error, {"workflow_id": "test-recovery", "business_critical": True}
        )
        ...
        for attempt in attempts:
            logger.info(
                f"  - {attempt.strategy.value}: {attempt.outcome.value} ({attempt.duration_seconds:.2f}s)"
            )
...
async def test_notification_service():
    ...
    try:
        ...
        test_error = DatabaseError(
            "Connection timeout", details={"host": "localhost", "timeout": 30}
        )

        await notification_service.send_error_notification(
            test_error, {"workflow_id": "test-notification", "user_id": "test-user"}
        )

        # Test recovery notification
        await notification_service.send_recovery_notification(
            "exponential_backoff", True, {"duration": 15.5, "attempts": 3}
        )
...
async def test_integration():
    ...
    try:
        ...
        logger.info(
            f"  - Dashboard health: {dashboard_data.get('health_indicators', {}).get('overall_status', 'unknown')}"
        )
        logger.info(
            f"  - Recovery success rate: {recovery_stats.get('success_rate', 0):.1f}%"
        )
        logger.info(
            f"  - Notifications sent: {notification_stats.get('total_notifications', 0)}"
        )
