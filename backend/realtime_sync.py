...
class ConflictResolver:
    ...
    async def resolve_conflict(self, events: List[SyncEvent], strategy: str = "last_write_wins"):
...
async def demonstrate_real_time_sync():
    ...
    await sync_engine.register_system("analytics_service", ["user_service", "order_service"])
    ...
    await sync_engine.event_stream.subscribe_to_stream("sync-user_service", handle_user_change)
    await sync_engine.event_stream.subscribe_to_stream("sync-order_service", handle_order_change)
    
    # Propagate changes
    await sync_engine.propagate_change("user_service", {
        "user_id": "user-123",
