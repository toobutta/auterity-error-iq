...
from app.services.relaycore_client import RelayCoreChatClient, RelayCoreFallbackHandler, RelayCoreFallbackMode
from app.services.ai_service_relaycore import EnhancedAIService, AIModelType
...
async def test_enhanced_ai_service():
    ...
    try:
        ...
        logger.info(f"Service health: RelayCore={health['relaycore']}, Direct={health['openai_direct']}")
        ...
        if template_response.is_success:
            logger.info(f"Template processing successful via {template_response.source}")
        ...
        direct_response = await service.process_text(
            prompt="Test direct OpenAI fallback",
            user_id="test-user",
            force_direct=True
        )
...
async def test_migration_wrapper():
    ...
    wrapper = AIServiceMigrationWrapper(
        use_relaycore=True,
    ...
    try:
        ...
        response = await wrapper.process_text(
            prompt="Test migration wrapper functionality",
            user_id="test-user"
        )
