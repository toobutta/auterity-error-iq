...
def cached_response(cache_key_prefix: str, ttl: int = None, vary_by: list = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if vary_by:
                for vary_param in vary_by:
                    if user_info and vary_param in user_info:
                        ...
                        cache_key_parts.append(f"{vary_param}:{getattr(request, vary_param)}")
            
            # Add function arguments to cache key
            cache_key = cache_manager._generate_cache_key(
                "_".join(cache_key_parts),
                *args,
                **{k: v for k, v in kwargs.items() if not isinstance(v, (Request, dict))}
            )
            ...
            # Cache the result
            cache_ttl = ttl or CacheConfig.CACHE_TTLS.get(cache_key_prefix, CacheConfig.DEFAULT_TTL)
