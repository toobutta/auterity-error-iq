...
class ErrorAnalyticsService:
    ...
    async def track_error(self, error: BaseAppException, context: Dict[str, Any] = None) -> None:
    ...
    async def _store_error_record(self, error_data: Dict[str, Any]) -> None:
        ...
        await self.redis.setex(key, int(self.metrics_retention.total_seconds()), 
    ...
    async def _check_recurring_patterns(self, error_data: Dict[str, Any]) -> None:
        ...
        recent_errors = await self._get_recent_errors_by_code(error_data['code'], hours=24)
    ...
    async def _get_recent_errors_by_code(self, error_code: str, hours: int = 1) -> List[Dict[str, Any]]:
    ...
    async def _get_errors_in_window(self, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
    ...
    async def _generate_insight(self, insight_data: Dict[str, Any]) -> None:
        ...
        await self.redis.setex(key, 86400 * 7, json.dumps(asdict(insight), default=str))  # 7 days
    ...
    async def get_error_metrics(self, period_hours: int = 24) -> ErrorMetrics:
        ...
        try:
            ...
            top_errors = [
                {"code": code, "count": count, "percentage": (count / total_errors * 100) if total_errors > 0 else 0}
    ...
    async def _calculate_error_trend(self, period_hours: int) -> ErrorTrend:
        ...
        try:
            ...
            current_errors = await self._get_errors_in_window(current_start, current_end)
