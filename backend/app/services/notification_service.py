...
class NotificationService:
    ...
    def _initialize_default_rules(self) -> None:
        ...
        self.notification_rules["recovery_status"] = NotificationRule(
            ...
            enabled=True,
            conditions={
                "recovery_actions": ["restart", "scale", "fallback"]
            },
    ...
    async def send_error_notification(
        ...
        try:
            ...
            for rule in applicable_rules:
                ...
                # Create notification
                notification = await self._create_error_notification(error, rule, context)
    ...
    async def send_recovery_notification(
        ...
        try:
            notification_type = (
                NotificationType.RECOVERY_SUCCESS if success 
            ...
            notification = Notification(
                ...
                priority=NotificationPriority.MEDIUM if success else NotificationPriority.HIGH,
    ...
    async def send_correlation_notification(
        ...
        try:
            ...
            if (confidence < rule.conditions.get("correlation_confidence", 0.7) or
                affected_systems < rule.conditions.get("affected_systems", 2)):
                return
    ...
    async def send_health_notification(
        ...
        try:
            ...
            priority = (
                NotificationPriority.CRITICAL if health_status == "critical"
    ...
    async def _check_error_spike_condition(self, conditions: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            prev_hour = (datetime.utcnow().replace(minute=0, second=0, microsecond=0) - 
    ...
    def _format_error_message(self, error: BaseAppException, context: Dict[str, Any]) -> str:
    ...
    def _format_health_message(self, health_status: str, metrics: Dict[str, Any]) -> str:
        ...
        if "error_rate" in metrics:
            message_parts.append(f"**Error Rate:** {metrics['error_rate']:.1f} errors/hour")
        ...
        if category_health:
            unhealthy_categories = [
                f"{cat}: {status}" for cat, status in category_health.items() 
                if status != "healthy"
            ]
            if unhealthy_categories:
                message_parts.append(f"**Affected Categories:** {', '.join(unhealthy_categories)}")
