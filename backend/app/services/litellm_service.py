...
class LiteLLMService:
    ...
    def __init__(
        ...
        config_path = config_path or os.getenv("MODEL_CONFIG_PATH", self.DEFAULT_CONFIG_PATH)
            ...
            if Path(config_path).exists():
                ...
                for model_data in config_data.get('models', []):
                    model_config = ModelConfig(
                        ...
                        api_key=model_data.get('api_key') or os.getenv(f"{model_data['provider'].upper()}_API_KEY"),
                ...
                self.logger.warning(f"Model configuration file not found: {config_path}")
        ...
        **kwargs
        ...
        for current_model in models_to_try:
            ...
            if model_config and not model_config.is_available:
                self.logger.warning(f"Model {current_model} is marked as unavailable, skipping")
                continue
