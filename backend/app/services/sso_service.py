...
class SSOService:
    ...
    async def initiate_saml_login(self, tenant_slug: str, relay_state: Optional[str] = None) -> str:
    ...
    async def handle_saml_response(self, saml_response: str, relay_state: Optional[str] = None) -> Dict:
    ...
    async def initiate_oidc_login(self, tenant_slug: str, state: Optional[str] = None) -> str:
    ...
    async def handle_oidc_callback(self, code: str, state: Optional[str] = None) -> Dict:
    ...
    def _generate_saml_authn_request(self, sso_config: SSOConfiguration, relay_state: Optional[str]) -> str:
...
<samlp:AuthnRequest xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"
    ...
    def _get_tenant_from_saml_data(self, user_data: Dict, relay_state: Optional[str]) -> Tenant:
    ...
    async def _authenticate_or_create_saml_user(self, tenant: Tenant, user_data: Dict) -> User:
        ...
        # Assign default role
        default_role = self.db.query(Role).filter(Role.name == sso_config.default_role).first()
        if default_role:
    ...
    async def _exchange_oidc_code(self, sso_config: SSOConfiguration, code: str) -> Dict:
    ...
    async def _authenticate_or_create_oidc_user(self, tenant: Tenant, user_data: Dict) -> User:
