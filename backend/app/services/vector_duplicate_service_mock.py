...
class MockVectorDuplicateService:
    ...
    async def find_similar_items(self, query_embedding: List[float], embeddings: List[Dict[str, Any]], threshold: float = 0.8) -> List[Dict[str, Any]]:
        ...
        for item in embeddings:
            similarity = await self.calculate_similarity(query_embedding, item["embedding"])
    ...
    async def detect_duplicates(self, tenant_id: UUID, content: str, item_type: str) -> Dict[str, Any]:
    ...
    async def create_similarity_cluster(self, tenant_id: UUID, items: List[Dict[str, Any]], threshold: float = 0.7) -> List[Dict[str, Any]]:
