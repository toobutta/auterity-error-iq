...
class SecurityManager:
    ...
    def _initialize_encryption(self) -> Fernet:
        ...
        password = self.config.get("encryption_password", "auterity-default-key").encode()
        ...
        for threat_type, patterns in self.threat_patterns.items():
            for pattern in patterns:
                if re.search(pattern, data_str, re.IGNORECASE):
                    ...
                    threat_level = max(threat_level, ThreatLevel.HIGH if threat_type in ["sql_injection", "command_injection"] else ThreatLevel.MEDIUM)
    ...
    async def get_security_metrics(self) -> Dict[str, Any]:
        ...
        recent_events = [
            ...
            if datetime.fromisoformat(event["timestamp"]) > datetime.now(timezone.utc) - timedelta(hours=24)
        ]
