...
class ExecutionManager:
    ...
    async def start_execution(
        ...
        logger.info(f"Started execution {execution_id} for {execution_type.value} {target_name}")
        return execution_id
    ...
    def list_executions(
        ...
        if execution_type:
            all_executions = [e for e in all_executions if e.execution_type == execution_type]
        if status:
    ...
    def get_child_executions(self, parent_execution_id: UUID) -> List[ExecutionLog]:
        ...
        all_executions = list(self.active_executions.values()) + self.execution_history
        return [e for e in all_executions if e.parent_execution_id == parent_execution_id]
    ...
    async def cancel_execution(self, execution_id: UUID) -> bool:
        ...
        return await self.update_execution_status(execution_id, ExecutionStatus.CANCELLED)
    ...
    def get_performance_stats(self, target_id: str, execution_type: ExecutionType) -> Dict[str, float]:
    ...
    def get_execution_analytics(
        ...
        successful_executions = len([e for e in filtered_executions if e.status == ExecutionStatus.COMPLETED])
        failed_executions = len([e for e in filtered_executions if e.status == ExecutionStatus.FAILED])
        ...
        for execution_type in ExecutionType:
            count = len([e for e in filtered_executions if e.execution_type == execution_type])
        ...
        for execution_type in ExecutionType:
            durations = [
                e.metrics.duration_ms for e in filtered_executions
                if e.execution_type == execution_type and e.metrics and e.metrics.duration_ms
            ]
            if durations:
                avg_duration_by_type[execution_type.value] = sum(durations) / len(durations)
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        for status in ExecutionStatus:
            count = len([e for e in self.active_executions.values() if e.status == status])
        ...
        for priority in ExecutionPriority:
            count = len([e for e in self.active_executions.values() if e.priority == priority])
