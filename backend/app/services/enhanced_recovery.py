...
from app.utils.retry_utils import RetryConfig, retry_async
...
class EnhancedRecoveryService:
    ...
    def _initialize_strategy_configs(self) -> Dict[RecoveryStrategy, Dict[str, Any]]:
        ...
        return {
            RecoveryStrategy.IMMEDIATE_RETRY: {
                "max_attempts": 3,
                "delay_seconds": 0,
                "applicable_categories": [ErrorCategory.AI_SERVICE, ErrorCategory.EXTERNAL_API],
            ...
            RecoveryStrategy.EXPONENTIAL_BACKOFF: {
                ...
                "base_delay": 1.0,
                "max_delay": 60.0,
                "applicable_categories": [ErrorCategory.DATABASE, ErrorCategory.EXTERNAL_API],
            ...
            RecoveryStrategy.GRACEFUL_DEGRADATION: {
                ...
                "applicable_categories": [ErrorCategory.AI_SERVICE, ErrorCategory.WORKFLOW],
    ...
    async def create_recovery_plan(
        ...
        try:
            ...
            success_probability = await self._estimate_success_probability(error, strategies)
        ...
        try:
            ...
            for i, strategy in enumerate(plan.strategies):
                ...
                if attempt.outcome == RecoveryOutcome.SUCCESS:
                    ...
                    self.logger.info(f"Partial recovery with strategy {strategy}, continuing")
                else:
            ...
            overall_success = any(a.outcome == RecoveryOutcome.SUCCESS for a in attempts)
            ...
            await notification_service.send_recovery_notification(
                ...
                overall_success,
                {
                    ...
                    "successful_strategy": next(
                        (a.strategy.value for a in attempts if a.outcome == RecoveryOutcome.SUCCESS),
                        None
                    )
                }
            )
    ...
    async def _get_historical_error_data(self, error_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_attempts = [a for a in historical_attempts if a.get("outcome") == "success"]
    ...
    async def _analyze_system_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_usage = await self._get_system_metric("memory_usage", 80.0)  # Default 80%
            error_rate = await self._get_system_metric("error_rate", 5.0)  # Default 5/hour
    ...
    async def _select_recovery_strategies(
        ...
        applicable_strategies = [
            strategy for strategy, config in self.strategy_configs.items()
        ...
        if system_state.get("under_stress", False):
            ...
            strategies = [s for s in low_impact_strategies if s in prioritized_strategies]
            strategies.extend([s for s in prioritized_strategies if s not in low_impact_strategies])
        else:
    ...
    async def _estimate_success_probability(
        ...
        for strategy in strategies:
            ...
            strategy_success_rate = await self._get_strategy_success_rate(strategy, error.category)
    ...
    async def _estimate_recovery_duration(self, strategies: List[RecoveryStrategy]) -> int:
    ...
    async def get_recovery_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_attempts = len([a for a in attempts if a["outcome"] == "success"])
            ...
            avg_duration = (
                sum(a["duration_seconds"] for a in completed_attempts) / len(completed_attempts)
