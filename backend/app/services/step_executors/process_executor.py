...
class ProcessStepExecutor(BaseStepExecutor):
        ...
        try:
            if not self.validate_input(input_data):
                return ExecutionResult(
                    success=False,
                    data={},
                    error="Invalid process input data"
                )
            ...
            processed_result = self._apply_processing_rules(data_to_process, processing_rules)
    ...
    def _apply_processing_rules(self, data: Dict[str, Any], rules: list) -> Dict[str, Any]:
