...
from .step_executors.base_executor import ExecutionResult
...
class RetryManager:
    ...
    async def handle_retry(self, context: ExecutionContext, error: Exception) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        if await self.should_retry(context, error):
            ...
            return ExecutionStatus.RETRYING, {"retry_count": context.retry_count, "delay": delay}
        
        return ExecutionStatus.FAILED, {"error": str(error), "final_retry_count": context.retry_count}
...
class TopologicalExecutor:
    ...
    def create_execution_plan(self, workflow_definition: Dict[str, Any]) -> ExecutionPlan:
    ...
    def _extract_dependencies(self, steps: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        for step_id, step_config in steps.items():
            ...
            dependencies[step_id] = deps if isinstance(deps, list) else [deps] if deps else []
        return dependencies
    
    def _topological_sort(self, steps: Set[str], dependencies: Dict[str, List[str]]) -> List[List[str]]:
...
class WorkflowExecutionEngine:
    ...
    async def execute_workflow(self, workflow_definition: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for batch_index, batch in enumerate(plan.execution_order):
                ...
                # Check for failures
                failed_steps = [step_id for step_id, (status, _) in batch_results.items() 
                              if status == ExecutionStatus.FAILED]
                
                if failed_steps:
                    logger.error(f"Batch {batch_index + 1} failed. Failed steps: {failed_steps}")
    ...
    async def _execute_batch(self, workflow_id: str, batch: List[str], plan: ExecutionPlan) -> Dict[str, Tuple[ExecutionStatus, Dict[str, Any]]]:
        ...
        for step_id in batch:
            ...
            context = ExecutionContext(
                ...
                input_data=self._prepare_step_input(workflow_id, step_id, step_config, plan.dependencies),
    ...
    async def _execute_step_with_semaphore(self, context: ExecutionContext) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        """Execute step with resource limiting"""
        async with self._semaphore:
            return await self._execute_step(context)
    
    async def _execute_step(self, context: ExecutionContext) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                executor = self.topological_executor.factory.create_executor(context.step_type)
                ...
                logger.warning(f"Step {context.step_id} attempt {attempt + 1} failed: {str(e)}")
                ...
                if attempt < max_attempts - 1:
                    ...
                    context.error = str(e)
                    return ExecutionStatus.FAILED, {"error": str(e), "retry_count": attempt}
        
        return ExecutionStatus.FAILED, {"error": "Max retries exceeded"}
    
    def _prepare_step_input(self, workflow_id: str, step_id: str, step_config: Dict[str, Any], dependencies: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    def get_execution_status(self, workflow_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_executions": len([ctx for ctx in self.active_executions.values() 
