    ...
    async def get_tenant_theme(self, tenant_id: UUID) -> Dict:
        ...
        try:
            ...
            theme = {
                ...
                "secondary_color": tenant.secondary_color or self.default_colors["secondary"],
    ...
    async def update_tenant_branding(
        ...
        try:
            ...
            if "primary_color" in branding_data:
                tenant.primary_color = self._validate_color(branding_data["primary_color"])
            
            if "secondary_color" in branding_data:
                tenant.secondary_color = self._validate_color(branding_data["secondary_color"])
            ...
            if "custom_css" in branding_data:
                tenant.custom_css = self._validate_custom_css(branding_data["custom_css"])
            
            if "remove_auterity_branding" in branding_data:
                tenant.remove_auterity_branding = bool(branding_data["remove_auterity_branding"])
            
            if "custom_domain" in branding_data:
                tenant.custom_domain = self._validate_custom_domain(branding_data["custom_domain"])
...
"""
    ...
    async def validate_branding_compliance(self, tenant_id: UUID) -> Dict:
        ...
        try:
            ...
            scores = [check.get('score', 0) for check in compliance_checks.values() if isinstance(check, dict)]
            if scores:
    ...
    def _validate_custom_css(self, css: str) -> str:
        ...
        dangerous_patterns = [
            r'javascript:', r'expression\(', r'url\(javascript:', r'behavior:',
            r'import\s+url', r'@import\s+url'
        ]
...
"""
    ...
    def _validate_css_compliance(self, css: str) -> Dict:
        ...
        if not css:
            return {"score": 100, "issues": [], "recommendations": ["No custom CSS provided"]}
        
        # Check for dangerous patterns
        dangerous_patterns = [
            r'javascript:', r'expression\(', r'behavior:'
        ]
    ...
    def _validate_domain_compliance(self, domain: str) -> Dict:
        ...
        if not domain:
            return {"score": 100, "issues": [], "recommendations": ["No custom domain configured"]}
