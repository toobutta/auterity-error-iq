...
from app.services.tool_registry import ToolRegistry, Tool, ToolType, ToolStatus
from app.services.protocol_manager import ProtocolManager, ProtocolMessage
from app.services.execution_manager import ExecutionManager, ExecutionType, ExecutionStatus, ExecutionPriority
from app.schemas.agent import AgentCreate, Agent
from app.schemas.mcp_server import MCPServerCreate, MCPServer
...
@agent_router.post("/register", response_model=Agent, status_code=status.HTTP_201_CREATED)
...
async def start_execution(execution_data: dict):
    ...
    try:
        execution_id = await execution_manager.start_execution(
            ...
            parent_execution_id=UUID(execution_data["parent_execution_id"]) if execution_data.get("parent_execution_id") else None,
