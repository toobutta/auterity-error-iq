...
from fastapi import APIRouter, HTTPException, Depends, status, BackgroundTasks, HTTPAuthorizationCredentials, Request
from fastapi.security import HTTPBearer
from pydantic import BaseModel, Field

# Import agent service classes
from backend.app.services.agents.compliance_layer import ComplianceLayer
from backend.app.services.agents.orchestrator import AgentOrchestrator
from backend.app.services.agents.rag_engine import RAGEngine
from backend.app.services.agents.security_manager import SecurityManager
...
try:
    ...
    from backend.app.core.cache import cached_response, invalidate_cache_on_mutation, cache_manager
    ...
    class AgentMetricsCollector:
        ...
        @staticmethod
        def record_workflow_execution(*args, **kwargs): pass
        @staticmethod
        def record_rag_query(*args, **kwargs): pass
        @staticmethod
        def record_compliance_validation(*args, **kwargs): pass
        @staticmethod
...
class AgentRegistrationRequest(BaseModel):
    ...
    agent_type: str = Field(
        ...
        regex=r"^(autmatrix|relaycore|neuroweaver)$"
    )
...
def handle_service_error(operation: str, error: Exception, user_id: Optional[str] = None) -> HTTPException:
...
async def register_agent(
    ...
    logger.info(f"Agent registration request from user {user_info.get('user_id')} for agent {request.agent_id}")
...
async def execute_workflow(
    ...
    _rate_limit: None = Depends(lambda req: rate_limiter.check_rate_limit(req, limit=10, window=60)),
):
    """Execute a workflow using registered agents (Rate limited: 10 requests/minute)"""
    
    start_time = time.time()
    logger.info(f"Workflow execution request from user {user_info.get('user_id')} for workflow {request.workflow_id}")
    ...
    try:
        ...
        logger.info(f"Successfully executed workflow {request.workflow_id} in {duration:.3f}s")
        ...
        if METRICS_AVAILABLE:
            AgentMetricsCollector.record_error("permission_denied", "/execute", user_info.get("tenant_id"))
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=str(e)
        )
        ...
        if METRICS_AVAILABLE:
            ...
            AgentMetricsCollector.record_error("execution_failed", "/execute", user_info.get("tenant_id"))
...
async def query_rag(
    ...
    logger.info(f"RAG query request from user {user_info.get('user_id')}: {request.query[:100]}...")
    ...
    try:
        ...
        logger.info(f"Successfully processed RAG query for user {user_info.get('user_id')}")
...
async def index_documents(
    ...
    logger.info(f"Document indexing request from user {user_info.get('user_id')}: {len(request.documents)} documents")
    ...
    try:
        ...
        logger.info(f"Successfully started document indexing for user {user_info.get('user_id')}")
...
async def validate_compliance(
    ...
    logger.info(f"Compliance validation request from user {user_info.get('user_id')} for operation: {request.operation}")
    ...
    try:
        ...
        logger.info(f"Compliance validation completed for user {user_info.get('user_id')}")
...
async def get_agent_status(
    ...
    try:
        ...
        if METRICS_AVAILABLE:
            AgentMetricsCollector.record_error("status_check", "/status", user_info.get("tenant_id"))
