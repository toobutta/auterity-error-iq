...
class RAGQueryRequest(BaseModel):
    ...
    domain: Optional[str] = Field(
        default=None, description="Domain-specific index to query"
    )
...
class DocumentIndexRequest(BaseModel):
    ...
    domain: str = Field(
        default="general", description="Domain classification for documents"
    )
...
def get_compliance_layer() -> ComplianceLayer:
    ...
    if not compliance_layer:
        config = {"compliance_level": "gdpr", "audit_retention_days": 365}
...
async def register_agent(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail=f"Failed to register agent {request.agent_id}"
            )
    ...
    try:
        ...
        if threat_result["threats_detected"]:
            ...
            if threat_result["action_required"]:
                raise HTTPException(
                    status_code=400, detail="Query contains potential security threats"
                )
