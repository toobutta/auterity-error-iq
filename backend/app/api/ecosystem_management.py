...
from app.core.relay_core import relay_core, RelayMessage, MessagePriority
    ...
    try:
        ...
        return {
            ...
            "ecosystem": {
                ...
                "cascade_failure_risk": ecosystem_analysis.get("cascade_failure_risk", 0.0),
                "services_count": len(ecosystem_analysis.get("services", {})),
                "optimization_opportunities": len(ecosystem_analysis.get("optimization_opportunities", []))
            },
            "relaycore": {
                ...
                "active_routes": relay_status.get("performance_metrics", {}).get("active_routes", 0),
            ...
            "neuroweaver": {
                ...
                "last_training_score": neuro_status.get("model_metrics", {}).get("accuracy", 0.0)
            },
            "ai_orchestration": {
        ...
        raise HTTPException(status_code=500, detail=f"Ecosystem status failed: {str(e)}")
        ...
        while True:
            try:
                ...
                live_data = {
                    ...
                    "neuro_training_active": neuro_status.get("current_phase") != "idle",
                    "ai_insights": {
                        "anomalies_detected": len([s for s in ecosystem_data.get("services", {}).values() 
                                                 if s.anomaly_score and s.anomaly_score > 0.5]),
                        "optimization_recommendations": len(ecosystem_data.get("auto_scaling_recommendations", {})),
                        "healing_actions_active": len(ecosystem_data.get("autonomous_healing_actions", []))
                    }
                }
...
async def start_relaycore():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start RelayCore: {str(e)}")
...
async def stop_relaycore():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to stop RelayCore: {str(e)}")
...
async def optimize_relaycore():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"RelayCore optimization failed: {str(e)}")


# ==================== NEUROWEAVER MANAGEMENT ====================
@router.post("/neuroweaver/train")
async def train_neuroweaver(training_request: TrainingRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        model_type = model_type_map.get(training_request.model_type, ModelType.REGRESSION)
...
async def trigger_autonomous_optimization(optimization_request: OptimizationRequest, 
...
async def get_ai_insights(service_name: str):
    ...
    try:
        ...
        insights = {
            ...
            "ai_analysis": {
                "health_score": await ai_orchestrator._calculate_health_score(health_data),
                "anomaly_score": await ai_orchestrator.anomaly_detector.detect_anomaly(service_name, health_data),
                "failure_probability": await ai_orchestrator.predictive_analytics.predict_failure_probability(service_name, health_data)
            },
            "predictions": await ai_orchestrator.predictive_analytics.get_comprehensive_prediction(service_name, health_data),
            "recommendations": await ai_orchestrator._generate_optimization_recommendations(service_name, health_data),
            "scaling_analysis": await ai_orchestrator.auto_scaler.analyze_scaling_needs(service_name, health_data),
            "timestamp": datetime.now().isoformat()
        }
...
async def get_capacity_predictions(days_ahead: int = 7):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Capacity predictions failed: {str(e)}")
...
async def detect_anomalies():
    ...
    try:
            ...
            for service_name in category_services.keys():
                ...
                if anomaly_score > 0.3:  # Lower threshold for comprehensive reporting
                    anomalies[f"{category}.{service_name}"] = {
                        ...
                        "recommended_actions": await ai_orchestrator._generate_optimization_recommendations(service_name, health_data)
                    }
        ...
        return {
            ...
            "system_stability": "stable" if len(anomalies) < 3 else "attention_required",
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
...
async def integrate_relay_neuro_optimization():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Integration optimization failed: {str(e)}")
...
async def check_integration_health():
    ...
    try:
        ...
        ai_orchestrator_healthy = len(ai_orchestrator.active_websockets) >= 0  # Always true if accessible
        
        integration_score = sum([relay_healthy, neuro_healthy, ai_orchestrator_healthy]) / 3
        
        return {
            "integration_health": "excellent" if integration_score > 0.8 else "good" if integration_score > 0.5 else "needs_attention",
...
async def get_performance_analytics():
    ...
    try:
        ...
        avg_health = sum(s.health_score for s in services.values() if s.health_score) / len(services) if services else 0
        
        return {
            "ecosystem_performance": {
                "average_health_score": avg_health,
                "total_services": len(services),
                "healthy_services": len([s for s in services.values() if s.health_score and s.health_score > 0.8]),
                "services_needing_attention": len([s for s in services.values() if s.health_score and s.health_score < 0.6])
            },
            "ai_performance": {
                "predictions_generated": len(ecosystem_data.get("predictive_insights", {})),
                "optimizations_recommended": len(ecosystem_data.get("auto_scaling_recommendations", {})),
                "anomalies_detected": len([s for s in services.values() if s.anomaly_score and s.anomaly_score > 0.5])
            },
            "system_efficiency": {
                "cascade_failure_risk": ecosystem_data.get("cascade_failure_risk", 0.0),
                "optimization_opportunities": len(ecosystem_data.get("optimization_opportunities", [])),
                "autonomous_actions_taken": len(ecosystem_data.get("autonomous_healing_actions", []))
            },
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
...
async def get_enhanced_services():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Enhanced services failed: {str(e)}")
