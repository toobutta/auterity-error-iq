...
class ServiceHealthChecker:
    ...
    async def check_service_http(self, name: str, url: str, expected_status: int = 200) -> Dict[str, Any]:
    ...
    async def check_database_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            import asyncpg
            conn = await asyncpg.connect("postgresql://postgres:password@postgres:5432/auterity")
    ...
    async def check_communication_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        services["twilio"] = {
            ...
            "status": "configured" if hasattr(settings, "TWILIO_ACCOUNT_SID") else "not_configured",
            "last_check": datetime.utcnow().isoformat()
        }
        
        services["whatsapp"] = {
            "name": "WhatsApp Business", 
            "status": "configured" if hasattr(settings, "WHATSAPP_ACCESS_TOKEN") else "not_configured",
            "last_check": datetime.utcnow().isoformat()
        }
    ...
    async def check_ai_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        services["openai"] = {
            "name": "OpenAI LLM",
            "status": "configured" if hasattr(settings, "OPENAI_API_KEY") else "not_configured",
            "last_check": datetime.utcnow().isoformat()
        }
        
        services["anthropic"] = {
            "name": "Anthropic Claude",
            "status": "configured" if hasattr(settings, "ANTHROPIC_API_KEY") else "not_configured", 
            "last_check": datetime.utcnow().isoformat()
        }
    ...
    async def check_infrastructure_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        services["kafka"] = await self.check_service_http(
            "Apache Kafka", "http://kafka:9092", expected_status=404  # Kafka doesn't have HTTP endpoint, expect 404
        )
    ...
    async def get_all_services_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_services = {
                ...
                "communication": results[2] if not isinstance(results[2], Exception) else {},
                "automation": results[3] if not isinstance(results[3], Exception) else {},
                ...
                "monitoring": results[6] if not isinstance(results[6], Exception) else {},
            ...
            return {
                "summary": {
                    ...
                    "unhealthy_services": total_services - healthy_services - configured_services,
                    "health_percentage": round((healthy_services + configured_services) / total_services * 100, 1) if total_services > 0 else 0,
                    "last_check": datetime.utcnow().isoformat()
                },
...
async def get_category_services_status(category: str):
    ...
    try:
        ...
        return {
            ...
            "healthy_count": len([s for s in result.values() if s.get("status") == "healthy"]),
            "last_check": datetime.utcnow().isoformat()
        }
    except Exception as e:
        await health_checker.close()
...
async def get_services_health_summary():
    ...
    overall_health = "healthy" if summary.get("health_percentage", 0) >= 90 else \
                    "degraded" if summary.get("health_percentage", 0) >= 70 else "critical"
