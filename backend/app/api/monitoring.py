...
async def get_performance_metrics(
    ...
    for i in range(hours):
        ...
        hour_executions = [
            ...
            if hour_start <= e.started_at < hour_end
        ]
        
        if hour_executions:
            hour_successful = len([e for e in hour_executions if e.status == ExecutionStatus.COMPLETED])
...
async def get_system_metrics(db: Session = Depends(get_db)):
    ...
    try:
        ...
        recent_executions = db.query(func.count(WorkflowExecution.id)).filter(
...
async def get_workflow_metrics(
    limit: int = Query(10, ge=1, le=50, description="Number of top workflows to return"),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        ).outerjoin(WorkflowExecution).filter(
            Workflow.is_active == True
        ).group_by(
