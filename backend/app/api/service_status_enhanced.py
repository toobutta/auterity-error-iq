...
from app.services.ai_orchestrator import ai_orchestrator, ServiceHealthMetrics
...
async def get_service_ai_insights(service_name: str):
    ...
    try:
        ...
        # AI Analysis
        insights["ai_analysis"]["health_prediction"] = await ai_orchestrator._calculate_health_score(health_data)
        insights["ai_analysis"]["anomaly_score"] = await ai_orchestrator.anomaly_detector.detect_anomaly(
            service_name, health_data
        )
        
        # Predictive analytics
        insights["predictive_analytics"] = await ai_orchestrator.predictive_analytics.get_comprehensive_prediction(
            service_name, health_data
        )
        
        # Generate recommendations
        insights["recommendations"] = await ai_orchestrator._generate_optimization_recommendations(
            service_name, health_data
        )
        
        # Calculate optimization score
        insights["optimization_score"] = await ai_orchestrator._calculate_optimization_score(health_data)
...
async def optimize_specific_service(service_name: str, optimization_request: ServiceOptimizationRequest):
    ...
    try:
        ...
        healing_result = await ai_orchestrator.service_healer.auto_heal_service(service_name, issue_type)
        ...
        raise HTTPException(status_code=500, detail=f"Service optimization failed: {str(e)}")


@router.get("/services/capacity/forecast")
async def get_capacity_forecast(forecast_request: CapacityForecastRequest = CapacityForecastRequest()):
    ...
    try:
        ...
        ecosystem_recommendations = await _generate_ecosystem_capacity_recommendations(forecasts)
            ...
            for service_name in category_services.keys():
                ...
                if anomaly_score > 0.5:  # Threshold for reporting anomalies
                    anomalies[f"{category}.{service_name}"] = {
                        ...
                        "recommended_action": await _get_anomaly_action(anomaly_score, health_data)
                    }
            ...
            for service_name in category_services.keys():
                ...
                if failure_probability > 0.3:  # Report services with >30% failure risk
                    failure_predictions[f"{category}.{service_name}"] = {
                        ...
                        "time_to_failure": await _estimate_time_to_failure(failure_probability),
                        "prevention_actions": await _get_prevention_actions(service_name, health_data)
                    }
    ...
    try:
                ...
                try:
                    ...
                    enhanced_services[category][service_name] = {
                        **service_data,
                        ...
                        "predicted_performance": "excellent" if ai_score > 0.9 else "good" if ai_score > 0.7 else "needs_attention"
                    }
                except Exception:
        ...
        return {
            ...
            "metadata": {
                ...
                "total_services": sum(len(cat_services) for cat_services in enhanced_services.values())
            }
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Enhanced services listing failed: {str(e)}")
...
async def get_production_services_enhanced():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Enhanced production services failed: {str(e)}")
...
async def _generate_ecosystem_capacity_recommendations(forecasts: Dict) -> List[str]:
    ...
    high_cpu_services = [name for name, forecast in forecasts.items() 
                        if max(forecast.get("cpu_forecast", [0])) > 80]
    
    if high_cpu_services:
        recommendations.append(f"Scale up services with high CPU forecast: {', '.join(high_cpu_services[:3])}")
