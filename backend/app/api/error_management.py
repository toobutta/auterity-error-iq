...
async def get_error_metrics(
    period_hours: int = Query(24, ge=1, le=168, description="Time period in hours (1-168)")
):
...
async def get_error_insights(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get error insights: {str(e)}")
...
async def get_recent_notifications(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get recent notifications: {str(e)}")
...
async def trigger_manual_recovery(
    ...
    try:
        ...
        attempts = await recovery_service.execute_recovery_plan(plan, mock_error, context or {})
...
async def export_error_report(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to export error report: {str(e)}")