    ...
    **kwargs,
    ...
    for attempt in range(1, config.max_attempts + 1):
        try:
            ...
            if not should_retry_exception(e, config.retry_exceptions, config.stop_exceptions):
                logger.warning(
...
def with_retry(config: Optional[RetryConfig] = None, context: Optional[Dict[str, Any]] = None):
...
class CircuitBreaker:
    ...
    def _on_failure(self, exception: Exception) -> None:
        ...
        if self.state == CircuitBreakerState.HALF_OPEN:
            ...
            self.state = CircuitBreakerState.OPEN
            logger.warning(f"Circuit breaker {self.name} transitioned to OPEN - failure threshold reached")
