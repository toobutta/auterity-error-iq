...
class LogFilter:
    ...
    def matches(self, entry: LogEntry) -> bool:
        ...
        if self.message_contains and self.message_contains.lower() not in entry.message.lower():
            return False
...
class LogAggregator:
    ...
    def get_metrics(
        ...
        # Count by component
        component_counts = Counter(entry.component for entry in entries if entry.component)
        ...
        # Get top errors
        error_entries = [entry for entry in entries if entry.level in [LogLevel.ERROR, LogLevel.CRITICAL]]
