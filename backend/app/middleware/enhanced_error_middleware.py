...
class EnhancedErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    async def _extract_request_context(self, request: Request, start_time: float) -> dict:
    ...
    async def _trigger_auto_recovery(
        ...
        try:
            ...
            if (error.severity.value in ["high", "critical"] or 
                context.get("business_critical", False)):
...
class ErrorMetricsMiddleware(BaseHTTPMiddleware):
    ...
    async def _track_error_metric(self, request: Request, response: Response) -> None:
        ...
        try:
            ...
            await analytics_service.redis.expire(f"http_error_{response.status_code}", 86400)
    ...
    async def _track_exception_metric(self, request: Request, exception: Exception) -> None:
        """Track exception metrics."""
        try:
            analytics_service = await get_error_analytics_service()
...
class HealthCheckMiddleware(BaseHTTPMiddleware):
    ...
    async def _perform_health_check(self) -> None:
        ...
        try:
            ...
            previous_status = await analytics_service.redis.get("previous_health_status")
