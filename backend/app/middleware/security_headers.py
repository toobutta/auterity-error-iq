...
class APISecurityEnhancer:
    ...
    def __init__(self):
        self.blocked_user_agents = {
            "curl", "wget", "python-requests", "postman"  # Add as needed
        }
        self.suspicious_patterns = [
            "select", "union", "drop", "delete", "insert",  # SQL injection
            "<script", "javascript:", "eval(",  # XSS patterns
            "../", "..\\",  # Directory traversal
        ]
    ...
    async def _security_checks(self, request: Request) -> Optional[JSONResponse]:
        ...
        if request.url.path.startswith("/api/agents"):
            if not request.headers.get("content-type") and request.method in ["POST", "PUT", "PATCH"]:
                return JSONResponse(
                    status_code=400,
...
security_headers_middleware = SecurityHeadersMiddleware(
    enable_hsts=True,  # Set to False for development
    enable_csp=True
)
