...
class RequestLoggingConfig:
    ...
    SENSITIVE_FIELDS = {
        ...
        "jwt", "api_key", "bearer", "auth", "credentials", "ssn", "credit_card",
        "encryption_password", "jwt_secret"
    }
    ...
    def mask_sensitive_data(data: Any, depth: int = 0) -> Any:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if any(sensitive in key.lower() for sensitive in RequestLoggingConfig.SENSITIVE_FIELDS):
                    masked[key] = "[MASKED]"
                else:
            ...
            return [SecurityFilter.mask_sensitive_data(item, depth + 1) for item in data[:10]]  # Limit list size
...
class RequestResponseLogger:
    ...
    async def log_request(self, request: Request) -> str:
        ...
        log_data = {
            ...
            "request": {
                ...
                "body": SecurityFilter.mask_sensitive_data(request_body) if request_body else None
            },
            "user_context": {
    ...
    async def log_response(self, correlation_id: str, response: Response, 
        ...
        if RequestLoggingConfig.LOG_RESPONSE_BODY and response_body:
            if len(response_body) <= RequestLoggingConfig.MAX_BODY_SIZE:
                ...
                logged_response_body = f"[RESPONSE_TOO_LARGE:{len(response_body)}_bytes]"
        ...
        log_data = {
            ...
            "response": {
                ...
                "body": SecurityFilter.mask_sensitive_data(logged_response_body) if logged_response_body else None
            },
            "performance": {
