...
from app.types.workflow import WorkflowNode, StepExecutionResult
...
class InputStepExecutor(StepExecutor):
    ...
    async def execute_step(self, node: WorkflowNode, input_data: Dict[str, Any]) -> StepExecutionResult:
        ...
        return StepExecutionResult(
            success=True,
            output_data={field_name: input_data}
        )
    ...
    async def execute_step(self, node: WorkflowNode, input_data: Dict[str, Any]) -> StepExecutionResult:
        try:
            ...
            if template_name:
                ...
                formatted_prompt = prompt.format(**input_data) if "{" in prompt else prompt
...
class DataValidationStepExecutor(StepExecutor):
    ...
    async def execute_step(self, node: WorkflowNode, input_data: Dict[str, Any]) -> StepExecutionResult:
        ...
        try:
            ...
            return StepExecutionResult(
                success=True,
                output_data={"validation_result": "success", "validated_data": input_data}
            )
