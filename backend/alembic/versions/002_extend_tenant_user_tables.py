...
def upgrade():
    ...
    op.add_column('tenants', sa.Column('industry_profile', sa.String(50), sa.ForeignKey('industry_profiles.id')))
    op.add_column('tenants', sa.Column('profile_settings', sa.JSON()))

    # Add preferred_profile and profile_customizations to users
    op.add_column('users', sa.Column('preferred_profile', sa.String(50), sa.ForeignKey('industry_profiles.id')))
