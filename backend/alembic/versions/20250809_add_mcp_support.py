...
def upgrade():
    ...
    agent_status_enum = sa.Enum('ACTIVE', 'INACTIVE', 'UNHEALTHY', 'MAINTENANCE', name='agentstatus')
    agent_status_enum.create(op.get_bind(), checkfirst=True)
    protocol_type_enum = sa.Enum('MCP', 'OPENAI_API', 'WEBSOCKET', 'CUSTOM', name='protocoltype')
    ...
    op.create_table(
        ...
        sa.Column('mcp_server_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('mcp_servers.id')),
    ...
    op.create_index('ix_protocol_messages_protocol_type', 'protocol_messages', ['protocol_type'])
    op.create_index('ix_protocol_messages_created_at', 'protocol_messages', ['created_at'])
    ...
    op.create_index('ix_workflow_contexts_execution_id', 'workflow_contexts', ['execution_id'])
    op.create_index('ix_workflow_contexts_shared_context_id', 'workflow_contexts', ['shared_context_id'])
    ...
    op.add_column('workflow_executions', sa.Column('primary_agent_id', postgresql.UUID(as_uuid=True)))
    op.add_column('workflow_executions', sa.Column('agent_execution_data', sa.JSON()))
    op.add_column('workflow_executions', sa.Column('multi_agent_coordination', sa.JSON()))
    op.create_index('ix_workflow_executions_primary_agent_id', 'workflow_executions', ['primary_agent_id'])


def downgrade():
    op.drop_index('ix_workflow_executions_primary_agent_id', table_name='workflow_executions')
