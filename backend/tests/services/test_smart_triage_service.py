...
class TestSmartTriageService:
    ...
    def service(self, mock_db):
        ...
        with patch('app.services.smart_triage_service.AIService') as mock_ai, \
             patch('app.services.smart_triage_service.VectorService') as mock_vector:
            
            mock_ai_instance = Mock()
            mock_ai_instance.generate_completion = AsyncMock(return_value='{"routing_decision": "bug_triage", "confidence_score": 0.85, "reasoning": "Contains bug-related keywords", "suggested_actions": ["Assign to dev team"]}')
    ...
    async def test_ai_triage_failure(self, service):
        ...
        service.ai_service.generate_completion = AsyncMock(side_effect=Exception("AI service error"))
    ...
    async def test_get_active_triage_rules(self, service, mock_db, mock_triage_rules):
        ...
        # Mock database query
        mock_db.query.return_value.filter.return_value.order_by.return_value.all.return_value = mock_triage_rules
    ...
    async def test_get_triage_accuracy(self, service, mock_db):
        ...
        mock_db.query.return_value.filter.return_value.scalar.side_effect = [100, 5, 0.85]
