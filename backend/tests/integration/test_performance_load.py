...
class TestWorkflowPerformance:
    ...
    def test_single_workflow_execution_performance(
        ...
        assert avg_time < 2.0, f"Average execution time {avg_time:.3f}s exceeds 2s threshold"
        assert max_time < 5.0, f"Maximum execution time {max_time:.3f}s exceeds 5s threshold"
        assert std_dev < 1.0, f"Standard deviation {std_dev:.3f}s indicates inconsistent performance"
    ...
    def test_concurrent_workflow_execution_load(
        ...
        with patch('app.services.ai_service.AIService.process_text') as mock_ai:
            ...
            def execute_workflow(execution_index):
                ...
                try:
                    ...
                    return {
                        ...
                        "execution_id": response.json().get("id") if response.status_code == 200 else None
                    }
            ...
            with ThreadPoolExecutor(max_workers=num_concurrent) as executor:
                futures = [executor.submit(execute_workflow, i) for i in range(num_concurrent)]
            ...
            avg_execution_time = statistics.mean([r["execution_time"] for r in successful_executions])
            ...
            assert avg_execution_time < 3.0, f"Average execution time {avg_execution_time:.3f}s exceeds 3s under load"
            assert throughput >= 5.0, f"Throughput {throughput:.2f} executions/sec below 5/sec threshold"
    ...
    def test_database_performance_under_load(
        ...
        for workflow_id in workflow_ids[:10]:  # Test first 10
            start_time = time.perf_counter()
            response = client.get(f"/api/workflows/{workflow_id}", headers=authenticated_headers)
        ...
        # Performance assertions
        assert list_time < 2.0, f"Workflow listing took {list_time:.3f}s, exceeds 2s threshold"
        assert avg_retrieval_time < 0.5, f"Average retrieval time {avg_retrieval_time:.3f}s exceeds 0.5s threshold"
    ...
    def test_memory_usage_under_load(
        ...
        with patch('app.services.ai_service.AIService.process_text') as mock_ai:
            ...
            for i in range(100):
                ...
                if i % 10 == 0:
                    ...
                    assert memory_increase < 100, f"Memory usage increased by {memory_increase:.2f}MB after {i} executions"
        ...
        assert total_memory_increase < 50, f"Total memory increase {total_memory_increase:.2f}MB exceeds 50MB threshold"
    ...
    def test_api_response_time_consistency(
        ...
        for method, endpoint, data in endpoints_to_test:
            ...
            assert avg_time < 1.0, f"Average response time for {endpoint} is {avg_time:.3f}s, exceeds 1s"
            assert std_dev < 0.5, f"Response time std dev for {endpoint} is {std_dev:.3f}s, indicates inconsistency"
    ...
    def test_maximum_concurrent_executions(
        ...
        with patch('app.services.ai_service.AIService.process_text') as mock_ai:
            ...
            with ThreadPoolExecutor(max_workers=max_concurrent) as executor:
                futures = [executor.submit(execute_workflow, i) for i in range(max_concurrent)]
                results = [future.result() for future in as_completed(futures)]
            ...
            assert success_rate >= 0.70, f"Success rate {success_rate:.2%} below 70% at maximum load"
    ...
    def test_large_workflow_definition_handling(
        ...
        for i in range(100):
            large_definition["nodes"].append({
                ...
                "data": {
                    "label": f"Node {i}",
                    "prompt": f"Process step {i} with input: {{input}}" if i % 2 == 0 else "Start"
                }
            })
        ...
        assert creation_time < 5.0, f"Large workflow creation took {creation_time:.3f}s, exceeds 5s"
        assert retrieval_time < 2.0, f"Large workflow retrieval took {retrieval_time:.3f}s, exceeds 2s"
