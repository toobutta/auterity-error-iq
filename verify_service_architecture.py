...
class ServiceArchitectureVerifier:
    ...
    def __init__(self):
        self.expected_services = [
            "kong", "nginx", "backend", "frontend", "postgres", "redis",
            "rabbitmq", "kafka", "zookeeper", "vault", "mlflow", "weaviate",
            "puppeteer", "mailhog", "celery-worker", "prometheus", "grafana",
            "alertmanager", "jaeger", "loki", "promtail", "node-exporter",
            "redis-exporter", "postgres-exporter", "minio"
        ]
    ...
    def check_service_health(self) -> Dict:
        ...
        for service, url in health_checks:
            code, stdout, stderr = self.run_command(f"curl -s -o /dev/null -w '%{{http_code}}' {url}")
            
            if code == 0 and stdout in ['200', '302']:
                healthy_services.append(service)
            else:
                unhealthy_services.append({"service": service, "url": url, "error": stderr})
    ...
    def check_monitoring_stack(self) -> Dict:
        ...
        monitoring_services = [
            ...
            "promtail", "node-exporter", "redis-exporter", "postgres-exporter"
        ]
    ...
    def verify_architecture(self) -> Dict:
        ...
        return {
            ...
            "summary": {
                ...
                "architecture_completeness": self.calculate_completeness(compose_check, container_check)
            }
        }

    def calculate_completeness(self, compose_check: Dict, container_check: Dict) -> float:
    ...
    def print_verification_report(self, result: Dict):
        ...
        print(f"      Running: {containers.get('running_count', 0)}/{summary['expected_services']}")
        ...
        print(f"      Config Completeness: {monitoring.get('config_completeness', 0):.1f}%")
    ...
    def print_recommendations(self, result: Dict):
        ...
        if missing_services:
            print(f"   • Add missing services to docker-compose.unified.yml: {', '.join(missing_services)}")
        
        # Unhealthy services
        unhealthy = checks['service_health'].get('unhealthy_services', [])
        if unhealthy:
            print(f"   • Fix unhealthy services: {', '.join([s['service'] for s in unhealthy])}")
        ...
        if completeness < 90:
            print(f"   • Architecture is {completeness}% complete - work on missing components")
